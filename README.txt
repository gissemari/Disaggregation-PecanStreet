...
strideTraining: the larger the strideTraining the less number of sequences in the training Set. If this is smaller than the sequenceLength, the sequences for the training are overlapped.
strideTest: if we test with overlapped sequences, we will have to average the output for certain points as they do it in Kelly's paper. Other option is to set the strideTest equal to the sequenceLength and to get just one prediction per timeStep (from one sequence).
